...
read -p "网站地址:" ping
ping $ping ;;
6) bash nasyt-linux-toolddos/ddos.sh ;;
7) bash nasyt-linux-tool/cc/cc.sh ;;
8) bash nasyt-linux-tool/bt/bt_install.sh ;;
10) bash <(curl -sSL https://linuxmirrors.cn/docker.sh) ;;
11) clear | bash <(curl -L gitee.com/TimeRainStarSky/TRSS_AllBot/raw/main/Install-Docker.sh) ;;
12) bash nasyt-linux-tool/tool/tool.sh ;;
0) exit 0 ;;
